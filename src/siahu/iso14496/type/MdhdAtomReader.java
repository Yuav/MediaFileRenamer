 ...
 * the media in this track (in seconds since midnight, Jan. 1, 1904, in UTC
 * time)
 * <p>
 * <code>modification_time</code> is an integer that declares the most recent
 * time the media in this track was modified (in seconds since midnight, Jan. 1,
 * 1904, in UTC time)
 * <p>
 * <code>timescale</code> is an integer that specifies the time-scale for this
 * media; this is the number of time units that pass in one second. For example,
 * a time coordinate system that measures time in sixtieths of a second has a
 * time scale of 60.
 * <p>
 * <code>duration</code> is an integer that declares the duration of this media
 * (in the scale of the timescale).
 * <p>
 * <code>language</code> declares the language code for this media. See ISO
 * 639-2/T for the set of three character codes. Each character is packed as the
 * difference between its ASCII value and 0x60. Since the code is confined to
 * being three lower-case letters, these values are strictly positive.
 * 
 * @author psiahu
 * 
 */
