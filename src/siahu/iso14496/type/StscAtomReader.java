 ...
 * the following table
 * <p>
 * <code>first_chunk</code> is an integer that gives the index of the first
 * chunk in this run of chunks that share the same samples-per-chunk and
 * sample-description-index; the index of the first chunk in a track has the
 * value 1 (the first_chunk field in the first record of this box has the value
 * 1, identifying that the first sample maps to the first chunk).
 * <p>
 * <code>samples_per_chunk</code> is an integer that gives the number of samples
 * in each of these chunks
 * <p>
 * <code>sample_description_index</code> is an integer that gives the index of
 * the sample entry that describes the samples in this chunk. The index ranges
 * from 1 to the number of sample entries in the Sample Description Box
 * 
 * @author psiahu
 * 
 */
