 ...
 * AudioSampleEntry and for metadata tracks, a MetaDataSampleEntry. Hint tracks
 * use an entry format specific to their protocol, with an appropriate name.
 * <p>
 * For hint tracks, the sample description contains appropriate declarative data
 * for the streaming protocol being used, and the format of the hint track. The
 * definition of the sample description is specific to the protocol.
 * <p>
 * Multiple descriptions may be used within a track.
 * <p>
 * The ‘protocol’ and ‘codingname’ fields are registered identifiers
 * that uniquely identify the streaming protocol or compression format decoder
 * to be used. A given protocol or codingname may have optional or required
 * extensions to the sample description (e.g. codec initialization parameters).
 * All such extensions shall be within boxes; these boxes occur after the
 * required fields. Unrecognized boxes shall be ignored.
 * <p>
 * If the ‘format’ field of a SampleEntry is unrecognized, neither the
 * sample description itself, nor the associated media samples, shall be
 * decoded.
 * <p>
 * The samplerate, samplesize and channelcount fields document the default audio
 * output playback format for this media. The timescale for an audio track
 * should be chosen to match the sampling rate, or be an integer multiple of it,
 ...
 * <p>
 * An optional BitRateBox may be present at the end of any MetaDataSampleEntry
 * to signal the bit rate information of a stream. This can be used for buffer
 * configuration. In case of XML metadata it can be used to choose the
 * appropriate memory representation format (DOM, STX).
 * <p>
 ...
 * the ‘pasp’ and ‘clap’ sample entry boxes, respectively. These are
 * both optional; if present, they over-ride the declarations (if any) in
 * structures specific to the video codec, which structures should be examined
 * if these boxes are absent.
 * <p>
 ...
 * should not be re-sampled more than once in the process of rendering, if at
 * all possible.
 * <p>
 ...
 * picture centre.
 * <p>
 * The leftmost/rightmost pixel and the topmost/bottommost line of the clean
 * aperture fall at:
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * The audio output format (samplerate, samplesize and channelcount fields) in
 * the sample entry should be considered definitive only for codecs that do not
 * record their own output configuration. If the audio codec has definitive
 * information about the output format, it shall be taken as definitive; in this
 * case the samplerate, samplesize and channelcount fields in the sample entry
 * may be ignored, though sensible values should be chosen (for example, the
