 ...
 * </pre>
 * <p>This box specifies the characteristics of a single track. Exactly one Track Header Box is contained in a track.
 * <p>In the absence of an edit list, the presentation of a track starts at the beginning of the overall presentation. An
 * empty edit is used to offset the start time of a track.
 * <p>The default value of the track header flags for media tracks is 7 (track_enabled, track_in_movie,
 ...
 * <p>The width and height in the track header are measured on a notional ‘square’ (uniform) grid. Track video data
...
public class TkhdAtomReader implements AtomReader {
	...
	public int read(DataInputStream dis, final int len) throws IOException {
		...
		BigInteger trackID = new BigInteger(Arrays.copyOfRange(buf, offset, offset+=4));
		...
		BigInteger layer = new BigInteger(Arrays.copyOfRange(buf, offset, offset+=2));
		...
		BigInteger alternateGroup = new BigInteger(Arrays.copyOfRange(buf, offset, offset+=2));
		...
		BigInteger widthx = new BigInteger(Arrays.copyOfRange(buf, offset, offset+=2));
		BigInteger widthy = new BigInteger(Arrays.copyOfRange(buf, offset, offset+=2));
		BigInteger heightx = new BigInteger(Arrays.copyOfRange(buf, offset, offset+=2));
		BigInteger heighty = new BigInteger(Arrays.copyOfRange(buf, offset, offset+=2));
		System.out.println("Width, Height = " + widthx + "." + widthy + ", " + heightx + "." + heighty);
