 ...
 * </pre>
 * 
 * <p>This box within a Media Box declares the process by which the media-data in the track is presented, and thus,
 * the nature of the media in a track. For example, a video track would be handled by a video handler.
 * 
 * <p>This box when present within a Meta Box, declares the structure or format of the 'meta' box contents.
 * 
 * <p>There is a general handler for metadata streams of any type; the specific format is identified by the sample
 ...
 * 
 *        <p>NOTE : MPEG-7 streams, which are a specific kind of metadata stream, have their own handler
 *        declared, documented in the MP4 file format [ISO/IEC 14496-14].
 *        
 *        <p>NOTE : metadata tracks are linked to the track they describe using a track-reference of type
 *        ‘cdsc’. Metadata tracks use a null media header (‘nmhd’), as defined in subclause 8.4.5.5.
 *
 * @author psiahu
 *
 */
public class HdlrAtomReader implements AtomReader {
	...
	public int read(DataInputStream dis, final int len) throws IOException {
		...
		String handlerType = new String(Arrays.copyOfRange(buf, offset, offset+=4));
		...
		String name = new String(Arrays.copyOfRange(buf, offset, offset+=namelength));
